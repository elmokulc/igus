...
void loop()
{
  ...
  if (Serial.available())
    commandInterpreter(&stepperMotor, &stepCntr);
  ...
  if (WK_MODE == MD_LINEAR)
  {
    ...
    if ((mMode != MOTOR_INIT) && (mMode != MOTOR_ABORTED))
    {
      ...
      if ((mMode != MOTOR_CHECKED) && (stepperMotor.getMotorState(POSITION_REACHED) == 0))
      {
        ...
        if (mMode == MOTOR_RUNNING_RIGHT)
          stepCntr.absSteps += stepCntr.nSteps;
        else if (mMode == MOTOR_RUNNING_LEFT)
          stepCntr.absSteps -= stepCntr.nSteps;
  ...
  else
  {
    ...
    else if (cmdString.equals(CMD_GOTORIGHT))
    {
      ...
      if (mMode == MOTOR_CHECKED)
      {
        ...
        if (WK_MODE == MD_LINEAR){
            if (pose->nSteps <= 0)
              Serial.println(ACK_INVALID);
            else if ((pose->absSteps + pose->nSteps) > LIMIT_STEP_MAX)
              Serial.println(ACK_OUTRANGE);
            else
            {
    ...
    else if (cmdString.equals(CMD_POSE))
    {
      ...
      if (mMode == MOTOR_INIT)
      {
        ...
        Serial.println(stepperMotor.getEncoderSteps());
    }
...
void MoveToInitialPosition(Umotor *motor, step_rcd *pose)
{
  ...
  if (!motor->getLimitSwitchState())
  {                           // Limit switch not pressed
    motor->runContinous(CCW); // Move carriage left
    while (!motor->getLimitSwitchState())
      ;                // Wait for limit switch pressed
...
void checkSteps(class Umotor *motor, step_rcd *pose)
{
  ...
  while (deltaSteps != 0)
  {
    ...
    if (deltaSteps > 0)
      motor->rotateSteps(deltaSteps, DIR_RIGHT, ACCELERATION_FAST, VELOCITY_FAST); // Move right
    else
      motor->rotateSteps(int32_t(-1) * deltaSteps, DIR_LEFT, ACCELERATION_SLOW, VELOCITY_SLOW); // Move left
